...
import com.dejava.shipDestroyer.model.Shot;
...
public class ProcessingService {
                ...
                if (canvas[templateY][templateX].getHp() != 0) {
                    ...
                    for (int canvasY = 0; canvasY < height; canvasY++) {
                        ...
                        for (int canvasX = 0; canvasX < width; canvasX++) {
                            ...
                            if (newY < 0 || newY > battlegroundSize) {
                                ok = false;
                                break;
                            }
                            if (newX < 0 || newX > battlegroundSize) {
                                ok = false;
                                break;
                            }
                            if (battleground[newY][newX] == "HIT" && canvas[canvasY][canvasX].getHp() == 0) {
                                ok = false;
                                break;
                            }
                            if (battleground[newY][newX] == "MISS" && canvas[canvasY][canvasX].getHp() != 0) {
                                ok = false;
                                break;
                            }
                        }
    ...
    public void onHitEvent(GameStartedEvent game, GameBoard board) {
        int boardX = board.getLastHit().getX();
        int boardY = board.getLastHit().getY();
        int battlegroundSize = board.getBattlegroundSize();
        ...
        board.setNextPossibleShot(new Shot(x, y, Shot.STATUS.KILL));
    }
}
